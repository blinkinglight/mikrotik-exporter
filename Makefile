# go run -ldflags "-X github.com/blinkinglight/mikrotik-exporter/cmd.version=6.6.7-BETA -X github.com/blinkinglight/mikrotik-exporter/cmd.shortSha=`git rev-parse HEAD`" main.go version
...
deploy: utils
	CGO_ENABLED=0 gox -os="linux freebsd netbsd" -arch="amd64 arm arm64 386" -parallel=4 -ldflags "$(LDFLAGS)" -output "dist/github.com/blinkinglight/mikrotik-exporter_{{.OS}}_{{.Arch}}"
